...
c2 <-
  ...
  transmute(
    author,
    Words = as.vector(scale(Words)),
    Sentences = as.vector(scale(Sentences)),
    Paragraphs = as.vector(scale(Paragraphs))) %>%
  gather(type, count, -author) %>%
  mutate(type = as.factor(type))

cadence_density_plot <-
  ggdensity(c2,
    x = 'count',
    add = 'mean',
    color = 'type',
...
d2 <-
  ...
  transmute(
    author,
    TTR = as.vector(scale(TTR)),
    MATTR500 = as.vector(scale(MATTR500)),
    MATTR1k = as.vector(scale(MATTR1k))) %>%
  gather(type, count, -author) %>%
  mutate(type = as.factor(type))
