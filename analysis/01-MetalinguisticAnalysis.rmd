...
libraries <- c('dplyr', 'ggpubr', 'psych', 'nortest', 'mvnormtest', 'devtools', 'mnmacros', 'biotools', 'tidyr')
# install.packages(libraries) # if needed
...
c2 <-
  metadata %>%
  merge(cadence) %>%
  select(author, Words, Sentences, Paragraphs)
...
ggplot(c2, aes(x=values)) + geom_density(aes(group=ind, colour=ind))
