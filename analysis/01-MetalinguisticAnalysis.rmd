...
libraries <- c('dplyr', 'ggpubr', 'psych', 'nortest', 'mvnormtest')
...
Based on the above evidence, we elect to procede with just the MATTR500.
We _could_ include both the TTR and the MATTR 500 (dur to the lower of the two corrlations), but that seems counter productive when the purpose of MATTR is to document normalise TTR.

Gather up all the dependent variables at intrest into a single frame.
...
ad.test(data[,'Words'])
cvm.test(data[,'Words'])
lillie.test(data[,'Words'])
pearson.test(data[,'Words'])
sf.test(data[,'Words'])
