...
libraries <- c('dplyr', 'tidyr', 'ggpubr', 'mvnormtest', 'biotools', 'psych', 'car')
# if needed
# install.packages(libraries)
x = lapply(libraries, require, character.only = TRUE)
...
Furtheremore, the results from the multivariate normality test reject the null hypothesis, so we find evidence that the distribution is not jointly normal ($W$ = `r round(unname(st$statistic), 3)`, $p$ `r pv2str(st$p.value)`).
...
As we can see from `boxM()`, the result is signifigant ($p$ `r pv2str(bm$p.value)`) so we reject the null hypothesis and say there is evedence to support the variance-covariance matrix being un-equal.
...
mv_tab <- multivariate_significance_table(model)
(st <- model$residuals %>% t() %>% shapiro.test())
```

As can be seen, _author_ is jointly statisticaly significant $F$(12, 349.53) = 327.1 ($p$ < .001) with reasonable model fit ($W$ = `r round(unname(st$statistic), 3)`, $p$ = `r round(st$p.value, 3)`) and Effect size Partial $\eta^{2}$ = `r (1 - (0.0013273)^(1/min(3, 12)))`.
