...
libraries <- c('dplyr', 'tidyr', 'ggpubr', 'mvnormtest', 'biotools', 'psych')
# if needed
...
As can be seen from Wilks, _author_ is jointly statisticaly significant $F$(`r mv_tab[2,4]`, `r mv_tab[2,5]`) = `r round(mv_tab[2,3], 3)` ($p$ `r apa_pvalue(mv_tab[2,6])`) with reasonable model fit ($W$ = `r round(unname(st$statistic), 3)`, $p$ `r apa_pvalue(st$p.value)`) and Effect size Partial $\eta^{2}$ = `r (1 - (0.0013273)^(1/min(3, 12)))`.
...
group_n = data %>% group_by(author) %>% count()
column_names <- c(
  sprintf('CD (N=%i) M(SD)', group_n$n[group_n$author == 'CD']),
  sprintf('JA (N=%i) M(SD)', group_n$n[group_n$author == 'JA']),
  sprintf('MT (N=%i) M(SD)', group_n$n[group_n$author == 'MT']),
  'F', 'P', '$\\eta^{2}$')


s1 <- lm_to_anova(lm(Words ~ author, data = data), type = c('III'))
s2 <- lm_to_anova(lm(Sentences ~ author, data = data), type = c('III'))
s3 <- lm_to_anova(lm(Paragraphs ~ author, data = data), type = c('III'))
s4 <- lm_to_anova(lm(MATTR1k ~ author, data = data), type = c('III'))
